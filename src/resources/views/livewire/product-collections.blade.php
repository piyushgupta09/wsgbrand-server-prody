<div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="product" class="form-label">Active Collections</label>
                            <select id="product" class="form-select" wire:model="selectedCollection" required>
                                <option value="">Select Collection Name</option>
                                @if ($productOptions->isNotEmpty())
                                    @foreach ($collections as $key => $value)
                                        <option value="{{ $value->id }}">{{ Str::title($value->type) }} | {{ Str::title($value->name) }}</option>
                                    @endforeach
                                @endif
                            </select>
    ...
    @if ($productCollections->isEmpty())
        <p class="lead p-3 text-center">This product is not added to any collection yet</p>
    @else    
        <table class="table table-sm">
            <thead class="table-dark">
                <tr>
                    <th class="small text-start ps-2">S. No.</th>
                    <th class="small text-start" colspan="2">Option Name (Color)</th>
                    <th class="small text-start">Collection Name</th>
                ...
                @foreach ($productCollections as $item)
                    <tr>
                        ...
                        <td class="text-start text-capitalize">
                            <img src="{{ $this->getProductOption($item->id)->getImage() }}" class="of-contain wh-120">
                        </td>
                        <td class="text-start text-capitalize">{{ $this->getProductOption($item->id)->name }}</td>
                        <td class="text-start text-capitalize">{{ $item->name }}</td>
                        <td class="text-end">
                            <button class="btn btn-sm"  wire:click.prevent="delete({{ $item->id }})">
